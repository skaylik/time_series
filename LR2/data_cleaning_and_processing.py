...
# СОЗДАЕМ ВЕРСИЮ С ИНДЕКСОМ ДЛЯ ВРЕМЕННЫХ РЯДОВ
print("\n=== ПОДГОТОВКА ДАННЫХ ДЛЯ ВРЕМЕННЫХ РЯДОВ ===")

# Создаем версию с индексом
processed_dataset_indexed = processed_dataset.copy()

# Преобразуем timestamp в datetime и устанавливаем как индекс
processed_dataset_indexed['timestamp'] = pd.to_datetime(processed_dataset_indexed['timestamp'])
processed_dataset_indexed = processed_dataset_indexed.set_index('timestamp')

# Проверяем тип индекса
print(f"Тип индекса: {type(processed_dataset_indexed.index)}")
print(f"Индекс является DatetimeIndex: {isinstance(processed_dataset_indexed.index, pd.DatetimeIndex)}")
print(f"Первые 3 даты индекса: {processed_dataset_indexed.index[:3].tolist()}")

# Сохраняем данные (два варианта - с индексом и без)
optimized_dataset_filename = 'preprocessed_exchange_rates_dataset.csv'
optimized_dataset_indexed_filename = 'preprocessed_exchange_rates_dataset_indexed.csv'

# Сохраняем версию без индекса (для просмотра)
processed_dataset.to_csv(optimized_dataset_filename, index=False)
# Сохраняем версию с индексом (для анализа временных рядов)
processed_dataset_indexed.to_csv(optimized_dataset_indexed_filename)
...
print(f"   - {optimized_dataset_filename} (без индекса, для просмотра)")
print(f"   - {optimized_dataset_indexed_filename} (с индексом, для анализа)")
...
print(f"\n🔍 ПРОВЕРКА ИНДЕКСА ДЛЯ ВРЕМЕННЫХ РЯДОВ:")
print(f"   Тип индекса: {type(processed_dataset_indexed.index)}")
print(f"   Индекс монотонный: {processed_dataset_indexed.index.is_monotonic_increasing}")
print(f"   Частота данных: {pd.infer_freq(processed_dataset_indexed.index)}")

print(f"\n📋 Первые 3 строки обработанных данных (с индексом):")
print(processed_dataset_indexed.head(3))

print(f"\n🔍 ФИНАЛЬНАЯ ПРОВЕРКА ПРОПУСКОВ:")
all_columns_are_clean = True
for column in processed_dataset_indexed.columns:
    missing_count = processed_dataset_indexed[column].isnull().sum()
    if missing_count == 0:
...
# === ВАЖНО: ПРИМЕР ИСПОЛЬЗОВАНИЯ ДЛЯ DECOMPOSITIONANALYZER ===
print(f"\n" + "="*60)
print("💡 ИНСТРУКЦИЯ ДЛЯ ИСПОЛЬЗОВАНИЯ В DECOMPOSITIONANALYZER:")
print("="*60)

print(f"\nСПОСОБ 1 - Использование Series с DatetimeIndex:")
print("```python")
print("from decomposition_analysis import DecompositionAnalyzer")
print("import pandas as pd")
print("")
print("# Загружаем данные с индексом")
print("df_indexed = pd.read_csv('preprocessed_exchange_rates_dataset_indexed.csv', index_col='timestamp', parse_dates=True)")
print("")
print("# Используем целевой столбец как Series")
print("target_series = df_indexed['jordanian_dinar_target']")
print("analyzer = DecompositionAnalyzer(target_series)")
print("```")

print(f"\nСПОСОБ 2 - Использование DataFrame с указанием колонок:")
print("```python")
print("from decomposition_analysis import DecompositionAnalyzer")
print("import pandas as pd")
print("")
print("# Загружаем данные без индекса")
print("df = pd.read_csv('preprocessed_exchange_rates_dataset.csv')")
print("")
print("# Используем DataFrame подход")
print("analyzer = DecompositionAnalyzer(")
print("    df,")
print("    date_column='timestamp',")
print("    value_column='jordanian_dinar_target'")
print(")")
print("```")

print(f"\n🔍 ПРОВЕРКА ГОТОВНОСТИ ДАННЫХ:")
print(f"   Целевая переменная: 'jordanian_dinar_target'")
print(f"   Тип данных с индексом: {type(processed_dataset_indexed['jordanian_dinar_target'])}")
print(f"   Тип индекса: {type(processed_dataset_indexed['jordanian_dinar_target'].index)}")
print(f"   Размер временного ряда: {processed_dataset_indexed['jordanian_dinar_target'].shape}")
...
# Возвращаем оба варианта данных для дальнейшего использования
